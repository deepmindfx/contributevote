...
import { getContributionById, hasContributed } from './contributions';
  ...
  withdrawalRequests.forEach(request => {
    if (request.status === 'pending' && request.deadline) {
      ...
      if (deadlineDate < now) {
        ...
        const contribution = getContributionById(request.contributionId);
        const totalContributors = contribution?.members.filter(memberId => 
          hasContributed(memberId, request.contributionId)).length || 0;
